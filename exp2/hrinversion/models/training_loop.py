...
import legacy
from exp2.comm.metrics import metric_main
...
def training_loop(
    ...
    from tl2.proj.fvcore import global_cfg, build_model
    ...
    if (resume_pkl is not None) and (rank == 0):
        if 'resume_pkl' in global_cfg:
            resume_pkl = global_cfg.resume_pkl
        if global_cfg.tl_resume:
            resume_pkl = f"{global_cfg.tl_resumedir}/ckptdir/resume/G_D_G_ema.pkl"
        print(f'Resuming from local file:\n\t {resume_pkl}')
        with dnnlib.util.open_url(resume_pkl) as f:
            resume_data = legacy.load_network_pkl(f)

        load_list = [('G', G), ('D', D), ('G_ema', G_ema)]
        for name, module in load_list:
            logger.info(f"Resume model {name}")
            Checkpointer(module).load_state_dict(resume_data[name].state_dict())
            # misc.copy_params_and_buffers(resume_data[name], module, require_all=False)

        if global_cfg.fix_G_ema_mapping:
            logger.info(f"fixing G_ema_mapping: copy G_ema mapping network to G's mapping.")
            # G.mapping.load_state_dict(G_ema.mapping.state_dict())
            Checkpointer(G.mapping).load_state_dict(G_ema.mapping.state_dict())
        freeze_blocks = global_cfg.G_cfg.synthesis_kwargs.get('freeze_blocks', ())
        if  len(freeze_blocks) > 0:
            logger.info(f"fixing synthesis blocks {freeze_blocks}: "
                        f"copy G_ema network to G.")
            # G.load_state_dict(G_ema.state_dict())
            Checkpointer(G).load_state_dict(G_ema.state_dict())

        torch_utils.load_models(save_dir=f"{global_cfg.tl_resumedir}/ckptdir/resume/",
                                model_dict=saved_models,
                                rank=rank)
        logger.info(tl2_utils.dict2string(state_dict))
    ...
    cur_nimg = state_dict['cur_nimg']
    cur_tick = state_dict['cur_tick']
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (real_img, real_c, gen_z, gen_c) in enumerate(zip(phase_real_img,
                                                                             phase_real_c,
                                                                             phase_gen_z,
                                                                             phase_gen_c)):
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and \
            ...
            images = torch.cat(
                [G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()

            max_to_keep_snap = tl2_utils.MaxToKeep.get_named_max_to_keep(name='snapshot')
            snap_dir = max_to_keep_snap.step_and_ret_circle_dir(
                root_dir=f"{global_cfg.tl_ckptdir}", info_msg=f"cur_tick: {cur_tick}\n"
                                                              f"cur_nimg: {cur_nimg}")
            save_image_grid(images, os.path.join(snap_dir, f'fakes.jpg'),
        ...
        if (network_snapshot_ticks is not None) and \
            ...
            if (len(metrics) > 0):
                ...
                FID_v = stats_metrics.get('FID', None)
                ...
                if rank == 0:
                    ...
                    # save snap model
                    snapshot_pkl = os.path.join(snap_dir, f'G_D_G_ema.pkl')
                    with open(snapshot_pkl, 'wb') as f:
                        pickle.dump(snapshot_data, f)
                    state_dict['cur_nimg'] = cur_nimg
                    state_dict['cur_tick'] = cur_tick
                    state_dict['cur_FID_v'] = FID_v
                    torch_utils.save_models(snap_dir, model_dict=saved_models, cfg=global_cfg)

                    resume_dir = f"{global_cfg.tl_ckptdir}/resume"
                    shutil.rmtree(resume_dir, ignore_errors=True)
                    shutil.copytree(snap_dir, resume_dir)

                    if FID_v and FID_v < state_dict['best_FID_v']:
                        best_dir = f"{global_cfg.tl_ckptdir}/best_fid"
                        shutil.rmtree(best_dir, ignore_errors=True)
                        shutil.copytree(snap_dir, best_dir)
                        state_dict['best_FID_v'] = FID_v
                        torch_utils.save_models(best_dir, model_dict=saved_models,
                                                cfg=global_cfg, info_msg=f"fid: {FID_v}", msg_mode='a')
