...
def report_metric(result_dict, run_dir=None, snapshot_pkl=None, cur_nimg=0):
    metric = result_dict['metric']
    assert is_valid_metric(metric)
    if run_dir is not None and snapshot_pkl is not None:
        snapshot_pkl = os.path.relpath(snapshot_pkl, run_dir)

    jsonl_line = json.dumps(dict(result_dict, snapshot_pkl=snapshot_pkl, timestamp=time.time(), cur_nimg=cur_nimg))
    print(jsonl_line)
...
@register_metric
def fid2048_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=2048)
    return dict(FID=fid)
