...
@click.option('--transform', help='Input crop/resize mode',
              type=click.Choice(['center-crop', 'center-crop-wide', 'pad_zero', 'pad_mirror']))
@click.option('--width', help='Output width', type=int)
@click.option('--height', help='Output height', type=int)
@click.option('--compress_png', help='', type=bool, default=False)
@click.option('--use_jpg', help='', type=bool, default=False)
...
def convert_dataset(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files, desc=dest):
        ...
        if use_jpg:
            img.save(image_bits, format='JPEG')
            archive_fname = f'{archive_fname.rstrip(".png")}.JPEG'
        else:
            if compress_png:
                img.save(image_bits, format='png')
            else:
